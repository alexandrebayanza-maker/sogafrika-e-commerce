  ...
  return (
    ...
    >
      <Link href={`/products/${product.slug}`} className="group block">
        <div className="relative rounded-xl overflow-hidden bg-dark-900/50 border border-dark-800/50 hover:border-primary-500/30 transition-all duration-300 hover:shadow-glow">
          {/* Image */}
          <div className="relative aspect-square overflow-hidden bg-dark-800">
            ...
            {/* Overlay Actions */}
            <div className="absolute inset-0 bg-dark-950/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3">
              <button
                onClick={handleAddToCart}
                disabled={product.stock_quantity === 0}
                className="p-3 rounded-full bg-primary-500 text-white hover:bg-primary-400 transition-all transform scale-0 group-hover:scale-100 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                className={`p-3 rounded-full transition-all transform scale-0 group-hover:scale-100 ${
                  wishlisted ? 'bg-red-500 text-white' : 'bg-dark-700 text-white hover:bg-dark-600'
                }`}
              >
          ...
          <div className="p-4 space-y-2">
            ...
            <div className="flex items-center justify-between gap-2 pt-2">
              ...
              <button
                onClick={handleAddToCart}
                disabled={product.stock_quantity === 0}
                className="p-2 rounded-lg bg-primary-500/10 text-primary-400 hover:bg-primary-500 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                title="Add to cart"
              >
